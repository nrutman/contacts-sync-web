...
class SyncListController extends AbstractController
{
    ...
    public function index(): Response
    {
        ...
        $syncLists = $this->syncListRepository->findByOrganization($organization);
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', sprintf('Sync list "%s" has been created.', $syncList->getName()));
    ...
    public function edit(Request $request, SyncList $syncList): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', sprintf('Sync list "%s" has been updated.', $syncList->getName()));
    ...
    public function delete(Request $request, SyncList $syncList): Response
    {
        ...
        if ($this->isCsrfTokenValid('delete-sync-list-'.$syncList->getId(), $token)) {
            $name = $syncList->getName();
            $this->entityManager->remove($syncList);
            $this->entityManager->flush();

            $this->addFlash('success', sprintf('Sync list "%s" has been deleted.', $name));
        }
    ...
    public function toggle(Request $request, SyncList $syncList): Response
    {
        ...
        if ($this->isCsrfTokenValid('toggle-sync-list-'.$syncList->getId(), $token)) {
            $syncList->setIsEnabled(!$syncList->isEnabled());
            $this->entityManager->flush();

            $status = $syncList->isEnabled() ? 'enabled' : 'disabled';
            $this->addFlash('success', sprintf('Sync list "%s" has been %s.', $syncList->getName(), $status));
        }
    ...
    public function history(Request $request, SyncList $syncList): Response
    {
        ...
        $syncRuns = $this->syncRunRepository->findBySyncList($syncList, $limit, $offset);
