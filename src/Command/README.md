...
| Command | Description |
|---------|-------------|
| `sync:run` | Syncs contacts from source to destination for all enabled lists (or a specific list via `--list`). Automatically refreshes source lists that support it; skip with `--no-refresh`. Delegates to `SyncService` with `trigger: 'cli'`. |
...
- **`source:refresh`** looks up the source provider from each sync list's source credential and calls its `refreshList()` method. Currently, only providers implementing `RefreshableProviderInterface` support this operation.
