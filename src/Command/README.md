...
| `app:setup` | Interactive first-run wizard. Walks through database connection, encryption key generation, email configuration, writes `.env.local`, runs migrations, imports existing `parameters.yml` config, and creates the first admin user. |
...
| `app:migrate-config` | One-time migration that reads `parameters.yml` and `var/google-token.json` and populates the database (`Organization`, `SyncList`, `InMemoryContact` entities). Prompts for confirmation if data already exists. |
...
- **`app:migrate-config`** runs inside a database transaction. If any step fails, nothing is committed. It reads secrets from Symfony's injected parameters (not raw YAML) and never writes them to files.
