...
- **PHP 8.3 compatibility is required** (as of 2026-03-17). The production server runs PHP 8.3. Do not upgrade dependencies to versions that require PHP 8.4+, and do not use PHP 8.4+ features (property hooks, asymmetric visibility, `array_find()`, etc.). The `platform.php` setting in `composer.json` enforces this at the dependency level.
- PHP >=8.3 with Symfony 7.2. Use constructor promotion and PHP 8+ features (attributes, named arguments, readonly properties, etc.) where appropriate.
